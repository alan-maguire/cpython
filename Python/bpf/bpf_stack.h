...
static __always_inline int
bpf_stack_append_str(struct bpf_stack_args *args, const char *str)
{
	int ret;
	...
	/* verifier */
	if (args->str_len >= sizeof(args->str) - BPF_STACK_MAX_CHUNK - 1)
		return -ENOSPC;
	ret = bpf_probe_read_user_str(args->str + args->str_len,
				      BPF_STACK_MAX_CHUNK, str);
	if (ret < 0)
		return ret;
	if (ret > 0) {
		args->str_len += ret - 1;
		args->str[args->str_len] = '\0';
	}
